...
def index(array, float_num):
    ...
    for n,k in enumerate(array):
        ...
        if(math.isclose(k, float_num) or (array[n-1] <= float_num <= k)):
            return n
...
def setrun(claw_pkg='geoclaw'):
    ...
    def points_on_shoreline(topo, track, num_of_frames):

        points = np.empty([int(num_of_frames/2), 2])

        count = 0

        for n in np.arange(0,num_of_frames,2):
            track_data = track.get_track(n)

            nearest_shore = near_shoreline(track_data[0], track_data[1], shoreline)
    ...
    # points_coast = points_on_shoreline(topo, track, 12)
  ...
  #  for n in points_coast:
   #     rundata.regiondata.regions.append([5,6,rundata.clawdata.t0,rundata.clawdata.tfinal, \
    #    n[0] + 1, n[0] -1, n[1] + 1, n[1] -1])


    water = []

    for i, n in enumerate(topo.Z):
   ...
   # for n in points_coast:
    ...
    #    topo_x = index(topo.x, n[0])
     #   topo_y = index(topo.y, n[1])

      #  try:
       #     water.index([topo_x, topo_y])
        #    points_gauges.append(n)
         #   print("In water")

          #  rundata.gaugedata.gauges.append([gauge_count, n[0] , n[1],
#                                             rundata.clawdata.t0,
